  ...
  tagList(
    sidebarPanel(
      conditionalPanel(
        ...
        # Species selector
      selectizeInput(inputId = nsZooSpatial('species'), label = "Select a zooplankton species", choices = unique(fMapData$Taxon), 
                     selected = "Acartia danae")
      ),
      conditionalPanel(
        condition="input.NRSspat == 2",  
        # Species selector
        selectizeInput(inputId = nsZooSpatial('species1'), label = "Select a copepod species", choices = unique(sti$Species), 
                       selected = "Acartia danae")
      )
    ),
    mainPanel(
