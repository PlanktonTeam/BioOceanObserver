...
mod_PolLTM_server <- function(id){
  moduleServer(id, function(input, output, session){
    ...
    gg_out1 <- reactive({
      ...
      p1 <- planktonr::pr_plot_EOV(outputs(), EOV = "Nitrate_umolL", Survey = "LTM", trans = "identity", col = col12[9], labels = "no")
      p2 <- planktonr::pr_plot_EOV(outputs(), EOV = "Phosphate_umolL", Survey = "LTM", trans = "identity", col = col12[11], labels = "no") 
      p4 <- planktonr::pr_plot_EOV(outputs(), EOV = "Silicate_umolL", Survey = "LTM", trans = "identity", col = col12[10], labels = "no") 
      p7 <- planktonr::pr_plot_EOV(outputs(), EOV = "Temperature_degC", Survey = "LTM", trans = "identity", col = col12[5], labels = "no")
      p3 <- planktonr::pr_plot_EOV(outputs(), EOV = "Salinity", Survey = "LTM", trans = "identity", col = col12[7])
