  ...
  shiny::tagList(
    tabsetPanel(id = "info", type = "pills",
                tabPanel("Technical Information", value = 2,
                         bslib::accordion(id = nsInfo("samplingAccordion_tech"), 
                              ...
                              The month term is wrapped in a harmonic function to ensure that the cyclic nature of months is accounted for, i.e. Jan 2023 follows Dec 2022.
                              The overall model significance and R squared values are also extracted from the model output and displayed on the plots.</p>")
                                            ),
                                          bslib::accordion_panel(
                ...
                tabPanel("References", value = 4,
                         bslib::accordion(id = nsInfo("samplingAccordion_refs"),
                                          open = character(0), # ensures all items are closed on render
                                         bslib::accordion_panel(
                ...
                tabPanel("Sampling Details", value = 5,
                         bslib::accordion(id = nsInfo("samplingAccordion_details"), 
                                          open = character(0), # ensures all items are closed on render
                                          bslib::accordion_panel(
                                                   title = shiny::HTML("<h3>National Reference Stations</h3>"),
                                                   value = 'nrs',
                                                   shiny::HTML("
                                                      <p>Regular biogeochemical sampling is conducted at 8 National Reference Stations (nrs) across Australia. These are mostly collected on a
                                      ...
                                      bslib::accordion_panel(
                                        ...
                                        value = 'cpr',
                                                      shiny::HTML("<p>The majority of the data in this App comes from the Australian Plankton Recorder Survey (AusCPR). 
                                                                  The regular survey tows for AusCPR are through GBR and down the East Australian Current across to Adelaide. 
                                                                  Adhoc tows are also completed from the RV Investigator, in the austral summer by RSV Nuyina, through a collaboration 
                                                                  with the Australian Antarctic Division (AAD), and from other research vessels. The phytoplankton data contains samples counted
                                                                  through UTAS / AAD and the Southern Ocean zooplankton records are counted through the SO_CPR program at the AAD</p> <br>"),
                ...
                tabPanel("Species Details", value = 6, 
                         bslib::accordion(id = nsInfo("samplingAccordion_species"), 
                                          open = character(0), # ensures all items are closed on render
                                          bslib::accordion_panel(
...
mod_info_server <- function(id){
  moduleServer( id, function(input, output, session){
    ...
    observeEvent(input$go_phyto, {
      session$sendCustomMessage("navigate", list("phyto1", "ptscpr"))
    })
