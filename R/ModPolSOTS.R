        ...
        shiny::conditionalPanel(
          ...
          shiny::checkboxGroupInput(inputId = nsPolSOTS("Parameters"), label = NULL, 
                                    choices = planktonr::pr_relabel(
                                      c("ChlF_mgm3", "DissolvedOxygen_umolL", "PhytoBiomassCarbon_pgm3",
                                    ...
                                    selected = c("ChlF_mgm3", "PhytoBiomassCarbon_pgL", "SST", "Nitrate_umolL", "Phosphate_umolL")),
          shiny::HTML("<strong>NOTE:</strong> Think about depths")
        ),
      ...
      shiny::mainPanel(
        ...
        shiny::tabsetPanel(id = "EOV_SOTS", type = "pills",
                           ...
                           shiny::tabPanel("Physical", value = 4,
                                           shiny::plotOutput(nsPolSOTS("timeseries4"), height = 2 * 200) %>%
                                             shinycssloaders::withSpinner(color="#0dc5c1"),
                                           div(style="display:inline-block; float:right; width:60%",
...
mod_PolSOTS_server <- function(id){
  moduleServer(id, function(input, output, session){
    ...
    titley <- planktonr::pr_relabel(c("ChlF_mgm3"), style = "ggplot")
    ...
    observeEvent({input$EOV_SOTS == 2}, {
      ...
      gg_out2 <- reactive({
        p1 <- planktonr::pr_plot_EOVs(selectedData(), EOV = "ChlF_mgm3", trans = "log10", col = col1["ChlF_mgm3"], labels = FALSE) 
        ...
        patchwork::wrap_elements(p1 / p2 / p4) &
          ggplot2::theme(title = ggplot2::element_text(size = 20, face = "bold"),
                         axis.title = ggplot2::element_text(size = 12, face = "plain"),
                         axis.text =  ggplot2::element_text(size = 10, face = "plain"),
                         plot.title = ggplot2::element_text(hjust = 0.5))
      ...
      gg_out3 <- reactive({
        ...
        p3 <- planktonr::pr_plot_EOVs(selectedData(), EOV = "Silicate_umolL", trans = "identity", col = col1["Silicate_umolL"], labels = FALSE)
         p4 <- planktonr::pr_plot_EOVs(selectedData(), EOV = "Phosphate_umolL", trans = "log10", col = col1["Phosphate_umolL"], labels = FALSE)
          p5 <- planktonr::pr_plot_EOVs(selectedData(), EOV = "DissolvedOxygen_umolL", trans = "identity", col = col1["DissolvedOxygen_umolL"])
       
        patchwork::wrap_elements(p2 / p3/ p4 / p5) &
          ggplot2::theme(title = ggplot2::element_text(size = 20, face = "bold"),
                         axis.title = ggplot2::element_text(size = 12, face = "plain"),
                         axis.text =  ggplot2::element_text(size = 10, face = "plain"),
                         plot.title = ggplot2::element_text(hjust = 0.5))
    ...
    observeEvent({input$EOV_SOTS == 4}, {
      gg_out4 <- reactive({
        p1 <- planktonr::pr_plot_EOVs(selectedData(), EOV = "SST", trans = "identity", col = col1["SST"], labels = FALSE)
