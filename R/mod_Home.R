...
mod_home_ui <- function(id){
  ...
  tagList(
    tabsetPanel(id = ns("home"), type = "pills",
                           ...
                           shiny::fluidPage(
                            ...
                            data uptake to solve real world problems."),
                               ...
                               shiny::HTML("The major categories of data we provide within the app are found across the top bar, and include
                                 microbial, phytoplankton, zooplankton, larval fish and environment (chemical) parameters. The
                                 <strong>Environmental Reporting</strong> section includes summary data that may be useful as data overviews
                                 for managers and policy makers. Within each section, the data is often designated by sampling regime,
                                 ...
                                 More information about the methods used in this tool can be found under the 'Technical Information' tab above."),
                               shiny::br(),
                               shiny::br(),
                               shiny::h3("Citation"),
                               ...
                               shiny::br(),
                               shiny::br(),
                               # Clear float to ensure footer doesn't wrap around the image
                               div(class = "clear-float")
                             )
                           ),
                           # Footer outside fluidPage so it can break out of the container padding
                           div(
                             class = "home-footer",
                             div(
                               ...
                               div(
                                 ...
                                 div(
                                   class = "funding-logos",
                                   shiny::tags$a(shiny::img(src = "www/IMOS_logo-stacked-Colour.png"),
                                                 href = "https://imos.org.au", target = "_blank"),
                                   shiny::tags$a(shiny::img(src = "www/ARDC_logo_RGB.png"),
                                                 href = "https://ardc.edu.au", target = "_blank"),
                                   shiny::tags$a(shiny::img(src = "www/BIO-RGB_Full-POS_crop.png"),
                                                 href = "https://bioplatforms.com", target = "_blank"),
                                   shiny::tags$a(shiny::img(src = "www/NCRIS-logo.png"),
                                                 href = "https://www.education.gov.au/ncris", target = "_blank"),
                                   shiny::tags$a(shiny::img(src = "www/nesp-marine-coastal.png"),
                                                 href = "https://www.nespmarinecoastal.edu.au", target = "_blank"),
                                   shiny::tags$a(shiny::img(src = "www/CSIRO_Logo.png"),
                                                 href = "https://www.csiro.au/en/about/people/research-units/environment", target = "_blank")
                                 )
                               )
                             )
                           )
                ...
                tabPanel("Sampling Details", value = 7,
                  bslib::accordion(
                ...
                tabPanel("Species Details", value = 8,
                  bslib::accordion(
...
mod_home_server <- function(id){
  moduleServer(id, function(input, output, session){
    ...
    # Close all accordion panels on startup
    session$onFlushed(function() {
      bslib::accordion_panel_close(id = ns("samplingAccordion_tech"),    value = c("issue", "wrangling", "stats", "faq", "change"))
      bslib::accordion_panel_close(id = ns("samplingAccordion_refs"),    value = c("refs", "cites"))
      bslib::accordion_panel_close(id = ns("samplingAccordion_details"), value = c("nrs", "cpr", "sots", "am", "hab"))
      bslib::accordion_panel_close(id = ns("samplingAccordion_species"), value = c("p", "z"))
    }, once = TRUE)

    output$progplot <- mapgl::renderMapboxgl({
      req(identical(input$home, "2"))
      fProgressMap(pkg.env$PMapData)
    })
    outputOptions(output, "progplot", suspendWhenHidden = FALSE)

    output$gantt <- shiny::renderPlot({
      req(identical(input$home, "3"))
    ...
    # ── Sampling Details tables (rendered when tab 7 is active) ──
    observeEvent(identical(input$home, "7"), {
    ...
    # ── Species Details tables (rendered when tab 8 is active) ──
    observeEvent(identical(input$home, "8"), {
